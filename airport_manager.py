...
def find_passengers_per_flight():
    ...
    for i in ticket_list:
        list1=i.split(":")
        list2.append(list1[0])  #add only flight_no that includes airline_name and some number
    
    for i in  list2:
        num=0
        string=''
        if i[:2] not in list3:  #slice only the airline_name 
            num=list2.count(i)  #add number
            list3.append(i[:2])  # add airline_name
            string+=i
            string+=":"
            string+=str(num) #concatinate
            list4.append(string) #add to new list
    return list4    
