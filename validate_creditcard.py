...
def validate_credit_card_number(card_number):
    ...
    list=[]
    for i in string:
        list.append(int(i))  #added to list
    sum=0
    for i in range(len(string)-2,-1,-2): #check from back 
        list[i]*=2  #double and store in list
        if list[i]>9:  #check is greater than 9
            list[i]=sum_digit(list[i])  #if yes then sum the digits
    for i in range(0,len(string)):  #sum of numbers in list
        sum+=list[i]
    if sum%10==0:  #if sum is divisible by 10 then true
        return True
    else:
        return False
